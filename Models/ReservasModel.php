...
class ReservasModel extends Mysql
{
	...
	public function insertDetalleReserva(string $codReserva, int $idServicio, float $precio, int $cantidad)
	{
	...
	public function deleteDetalleReserva(string $codReserva)
	{
		$query_delete = "DELETE FROM detalle_reserva WHERE cod_reserva = '" . $codReserva . "'";
		$request_delete = $this->delete($query_delete);
		return $request_delete;
	}
	public function getUltimoCodigoReserva()
	{
		$sql = "SELECT cod_reserva FROM reserva ORDER BY idReserva DESC LIMIT 1";
		$request = $this->select($sql);
		return $request;
	}
